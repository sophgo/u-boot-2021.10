...
static int ytphy_modify_ext(struct phy_device *phydev, u16 regnum, u16 mask, u16 set)
{
	...
	ret = phy_read(phydev, MDIO_DEVAD_NONE, YTPHY_PAGE_DATA);
	if (ret < 0)
		return ret;

	return phy_write(phydev, MDIO_DEVAD_NONE, YTPHY_PAGE_DATA, (ret & ~mask) | set);
}
...
static int yt8531_config(struct phy_device *phydev)
{
	...
	if (!ofnode_valid(node)) {
		printf("Failed to get device node\n");
		return -EINVAL;
	}
...
static struct phy_driver motorcomm8511_driver = {
...
{
	phy_register(&motorcomm8511_driver);
