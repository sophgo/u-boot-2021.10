...
unsigned int _MAKECOLOR(unsigned char r, unsigned char g, unsigned char b, enum SW_I80_FORMAT fmt)
{
...
int i80_set_combo_dev_cfg(const SW_I80_CFG_S *i80_sw_cfg)
{
	int ret;
	unsigned int clkkHz;
	...
	_fill_disp_timing(&timing, &i80_sw_cfg->sync_info);
	...
	i80_ctrl[I80_CTRL_CMD] = BIT(i80_sw_cfg->lane_s.RD) | ((BIT(i80_sw_cfg->lane_s.RD) | BIT(i80_sw_cfg->lane_s.WR)) << 4);
	i80_ctrl[I80_CTRL_DATA] = (BIT(i80_sw_cfg->lane_s.RD) | BIT(i80_sw_cfg->lane_s.RS)) |
				  ((BIT(i80_sw_cfg->lane_s.RD) | BIT(i80_sw_cfg->lane_s.WR) | BIT(i80_sw_cfg->lane_s.RS)) << 4);
	i80_ctrl[I80_CTRL_EOF] = 0xff;

	clkkHz = 1000000 / (i80_sw_cfg->cycle_time / 2);
	if (clkkHz < 8000) {
