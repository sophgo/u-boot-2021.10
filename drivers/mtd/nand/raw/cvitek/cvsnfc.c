...
//		// for dma write
		...
		writel(0x2A << (ch * 8), remap_base); //PER dst => remap chx
		...
		writel(0x2A << (ch * 8), remap_base); //PER src => remap chx
		uint32_t val = readl(remap_base) | 0x80000000; // update bit
...
static void cv_spi_nand_adjust_freq(struct cvsnfc_host *host, uint8_t sck_l, uint8_t sck_h, uint16_t prd)
{
	uint32_t val;

	cvsfc_write(host, REG_SPI_NAND_BOOT_CTRL,
			    ((cvsfc_read(host, REG_SPI_NAND_BOOT_CTRL) & BIT_REG_RSP_DLY_MASK) | prd));
...
int spi_nand_adjust_max_freq(struct cvsnfc_host *host, uint32_t *pattern1, uint32_t *pattern2)
{
	...
	prd = cvsfc_read(host, REG_SPI_NAND_BOOT_CTRL) & BIT_REG_RSP_DLY_MASK;
...
freq_retry:
	...
	cv_spi_nand_adjust_freq(host, host->nand_chip_info->sck_l, host->nand_chip_info->sck_h, prd);
