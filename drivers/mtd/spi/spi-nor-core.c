...
static int mx_read_cr_quad_enable(struct spi_nor *nor)
{
	u8 sr_cr[2] = { 0 };
	int ret;
	...
	if (ret & SR_QUAD_EN_MX)
		return 0;

	sr_cr[0] = ret | SR_QUAD_EN_MX;
			       ...
			       struct spi_nor_flash_parameter *params)
{
	...
	if (params->hwcaps.mask & (SNOR_HWCAPS_READ_QUAD |
		...
		case SNOR_MFR_ISSI:
			params->quad_enable = quad_enable_SR_bit6;
			if (info->id[1] == 0x25) {
				params->quad_enable = mx_read_cr_quad_enable;
				break;
			}
			break;
		case SNOR_MFR_EON:
