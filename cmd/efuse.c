...
CVI_S32 CVI_EFUSE_EnableFastBoot(void)
{
	...
	if (((chip & 0xFFF0F) == 0x1810C || (chip & 0xFFF0F) == 0x1840C) && ((chip >> 4) & 0xF) <= 3) { // 181XC (X <= 3)
...
#if IS_ENABLED(CONFIG_TARGET_CVITEK_CV181X) || IS_ENABLED(CONFIG_TARGET_CVITEK_CV184X)
	if (sel == CVI_EFUSE_SECUREBOOT_SIGN_ENCRYPT) {
